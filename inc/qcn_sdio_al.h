...
enum sdio_al_lpm_event {
	...
	LPM_RUNTIME_SUSPEND_ENTER, /* SDIO client will be put to Runtime Suspend mode soon */
	LPM_RUNTIME_SUSPEND_EXIT,  /* SDIO client has exited Runtime Suspend mode */
	LPM_RESET,
};
...
struct sdio_al_client_data {
	...
	int (*fw_assert_cb)(struct device *dev);
};
...
/**
 * qti_client_is_ready - API to know whether the qti client driver is ready
 * This API can be used to defer the probe incase of early execution.
 *
...
/**
 * sdio_al_process_pending_irq - process pending sdio irq (if any)
 *
 * @return none
 */
...
/**
 * sdio_al_send_host_trans_len - Fill data length to be transfered
 *
 * @data_len: lenght to be transferred
 *
 * @return 0 on success, non-zero in case of error
 */
