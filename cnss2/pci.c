                                       ...
                                       const struct pci_device_id *id_table)
{
	while (id_table && id_table->device) {
		if (plat_env && (plat_env->device_id == id_table->device))
			return true;
		id_table++;
	}
...
cnss_get_plat_priv_when_register_driver(struct cnss_wlan_driver *driver_ops)
{
	...
	for (i = 0; i < plat_env_count; i++) {
		 ...
		 * like pld_pcie_xxx_cnss2 and driver_ops_name also has sub string
		 * cnss2 like pld_pcie_xxx_cnss2
		 * also make sure plat_env has same device_id with driver_ops
		 */
		if (plat_env->pld_bus_ops_name) {
			if ((strstr(plat_env->pld_bus_ops_name, SECOND_DRIVER_SUB_NAME) != NULL)
			    && (strstr(driver_ops->name, SECOND_DRIVER_SUB_NAME) != NULL)
		...
		* also make sure plat_env has same device_id with driver_ops
		*/
			if ((strstr(driver_ops->name, SECOND_DRIVER_SUB_NAME) == NULL)
			    && cnss_pci_device_id_matched(plat_env, driver_ops->id_table))
				return plat_env;
		}
	}
